...
import { getAddress as getAddress3, parseErc6492Signature as parseErc6492Signature3, isAddressEqual as isAddressEqual2 } from "viem";
...
async function verifyEip3009DepositAuthorization(signer, payload, requirements, chainId, allowedFactories = []) {
  ...
  const hasDeploymentInfo = !!(factory && factoryCalldata && !isAddressEqual(factory, ZERO_ADDRESS));
...
async function deployErc3009CounterfactualIfNeeded(signer, payload, requirements, allowedFactories) {
  ...
  const { address: factory, data: factoryCalldata } = parseErc6492Signature3(auth.signature);
  const hasDeploymentInfo = !!(factory && factoryCalldata && !isAddressEqual2(factory, ZERO_ADDRESS2));
  if (!hasDeploymentInfo) {
    return null;
  }
  let code;
...
import { getAddress as getAddress5, isAddressEqual as isAddressEqual3, parseEventLogs } from "viem";
  ...
  try {
    ...
    if (receipt.logs) {
      const logs = parseEventLogs({
        abi: batchSettlementABI,
        eventName: "Settled",
        logs: receipt.logs.filter((log) => isAddressEqual3(log.address, contractAddr))
      });
      const settledLog = logs.find(
        (log) => isAddressEqual3(log.args.receiver, receiver) && isAddressEqual3(log.args.token, token)
      );
...
async function executeRefundWithSignature(signer, payload, requirements, authorizerSigner, dataSuffix) {
  ...
  try {
    ...
    if (!hasClientSig && authorizerSigner && getAddress6(payload.channelConfig.receiverAuthorizer) !== getAddress6(authorizerSigner.address)) {
      return {
        success: false,
        errorReason: ErrAuthorizerAddressMismatch,
        transaction: "",
