...
async function classifyErc6492Payer(signer, signature, payerAddress) {
  ...
  const hasDeploymentInfo = !!(erc6492Data.address && erc6492Data.data && !(0, import_viem6.isAddressEqual)(erc6492Data.address, ZERO_ADDRESS));
...
async function settleEIP3009(signer, payload, requirements, eip3009Payload, config, context) {
  ...
  try {
    const settleErc6492Data = (0, import_viem9.parseErc6492Signature)(eip3009Payload.signature);
    const {
    ...
    const settlePayload = erc6492InnerSig && erc6492InnerSig !== eip3009Payload.signature ? { ...eip3009Payload, signature: erc6492InnerSig } : eip3009Payload;
    ...
    if (receipt.logs != null && !verifyEip3009TransferEvent(receipt.logs, (0, import_viem9.getAddress)(requirements.asset), {
      from: (0, import_viem9.getAddress)(auth.from),
      to: (0, import_viem9.getAddress)(auth.to),
...
async function validateErc20ApprovalForPayment(info, payer, tokenAddress) {
  ...
  try {
    ...
    const tx = (0, import_viem10.parseTransaction)(serializedTx);
    if (!tx.to || (0, import_viem10.getAddress)(tx.to) !== tokenAddress) {
      return {
        isValid: false,
    ...
    try {
      const decoded = (0, import_viem10.decodeFunctionData)({
        abi: erc20ApproveAbi,
        data
      });
      const calldataSpender = (0, import_viem10.getAddress)(decoded.args[0]);
      if (calldataSpender !== (0, import_viem10.getAddress)(PERMIT2_ADDRESS)) {
        return {
          isValid: false,
    ...
    try {
      const recoveredAddress = await (0, import_viem10.recoverTransactionAddress)({
        serializedTransaction: serializedTx
      });
      if ((0, import_viem10.getAddress)(recoveredAddress) !== (0, import_viem10.getAddress)(payer)) {
        return {
          isValid: false,
...
function validateEip2612PermitForPayment(info, payer, tokenAddress) {
  ...
  if ((0, import_viem11.getAddress)(info.from) !== (0, import_viem11.getAddress)(payer)) {
    return { isValid: false, invalidReason: ErrEip2612FromMismatch };
  }
  if ((0, import_viem11.getAddress)(info.asset) !== tokenAddress) {
    return { isValid: false, invalidReason: ErrEip2612AssetMismatch };
  }
  if ((0, import_viem11.getAddress)(info.spender) !== (0, import_viem11.getAddress)(PERMIT2_ADDRESS)) {
...
async function simulatePermit2SettleWithErc20Approval(config, extensionSigner, settleArgs, erc20Info) {
  ...
  try {
    const settleData = (0, import_viem11.encodeFunctionData)({
      abi: config.proxyABI,
      functionName: "settle",
      args: settleArgs
    });
...
function buildExactPermit2SettleArgs(permit2Payload) {
  const { signature } = (0, import_viem11.parseErc6492Signature)(permit2Payload.signature);
  return [
    {
      permitted: {
    ...
    (0, import_viem11.getAddress)(permit2Payload.permit2Authorization.from),
    {
...
async function verifyPermit2(signer, payload, requirements, permit2Payload, context, options) {
  ...
  const tokenAddress = (0, import_viem12.getAddress)(requirements.asset);
  const assetBytecode = await signer.getCode({ address: tokenAddress });
  if (!assetBytecode || assetBytecode === "0x") {
    return { isValid: false, invalidReason: ErrAssetNotDeployedContract, payer };
  }
  if ((0, import_viem12.getAddress)(permit2Payload.permit2Authorization.spender) !== (0, import_viem12.getAddress)(x402ExactPermit2ProxyAddress)) {
    return {
      isValid: false,
  ...
  if ((0, import_viem12.getAddress)(permit2Payload.permit2Authorization.witness.to) !== (0, import_viem12.getAddress)(requirements.payTo)) {
    return {
      isValid: false,
  ...
  if ((0, import_viem12.getAddress)(permit2Payload.permit2Authorization.permitted.token) !== tokenAddress) {
    return {
      isValid: false,
  ...
  const permit2TypedData = {
    ...
    message: {
      permitted: {
        token: (0, import_viem12.getAddress)(permit2Payload.permit2Authorization.permitted.token),
        amount: BigInt(permit2Payload.permit2Authorization.permitted.amount)
      },
      spender: (0, import_viem12.getAddress)(permit2Payload.permit2Authorization.spender),
      nonce: BigInt(permit2Payload.permit2Authorization.nonce),
      deadline: BigInt(permit2Payload.permit2Authorization.deadline),
      witness: {
        to: (0, import_viem12.getAddress)(permit2Payload.permit2Authorization.witness.to),
        validAfter: BigInt(permit2Payload.permit2Authorization.witness.validAfter)
      }
    }
  };
...
async function settlePermit2WithERC20Approval(config, extensionSigner, payload, permit2Payload, erc20Info, dataSuffix) {
  ...
  try {
    const settleData = appendDataSuffix(
      (0, import_viem12.encodeFunctionData)({
        abi: config.proxyABI,
        functionName: "settle",
...
var ExactEvmSchemeV12 = class {
  ...
  async settle(payload, requirements, context) {
    ...
    try {
      const { address: factoryAddress, data: factoryCalldata } = (0, import_viem14.parseErc6492Signature)(
        exactEvmPayload.signature
      );
      if (factoryAddress && factoryCalldata && !(0, import_viem14.isAddressEqual)(factoryAddress, "0x0000000000000000000000000000000000000000")) {
      ...
      const tx = await executeTransferWithAuthorization(
        this.signer,
        (0, import_viem14.getAddress)(requirements.asset),
        exactEvmPayload,
        dataSuffix
      );
      ...
      if (receipt.logs != null && !verifyEip3009TransferEvent(receipt.logs, (0, import_viem14.getAddress)(requirements.asset), {
        from: (0, import_viem14.getAddress)(auth.from),
        to: (0, import_viem14.getAddress)(auth.to),
        value: BigInt(auth.value)
      })) {
        return {
          success: false,
  ...
  async _verify(payload, requirements, options) {
    ...
    const { name, version } = requirements.extra;
    const erc20Address = (0, import_viem14.getAddress)(requirements.asset);
    ...
    if ((0, import_viem14.getAddress)(exactEvmPayload.authorization.to) !== (0, import_viem14.getAddress)(requirements.payTo)) {
      return {
        isValid: false,
        invalidReason: ErrRecipientMismatch,
        payer
