...
__export(src_exports, {
  ...
  classifyErc6492Payer: () => classifyErc6492Payer,
  ...
  getERC7702DelegateAddress: () => getERC7702DelegateAddress,
  ...
  isERC7702Delegation: () => isERC7702Delegation,
  ...
  verifyHashSignatureWithCode: () => verifyHashSignatureWithCode,
...
async function classifyErc6492Payer(signer, signature, payerAddress) {
  const erc6492Data = (0, import_viem11.parseErc6492Signature)(signature);
  const hasDeploymentInfo = !!(erc6492Data.address && erc6492Data.data && !(0, import_viem11.isAddressEqual)(erc6492Data.address, ZERO_ADDRESS));
...
async function verifyTypedDataSignature(signer, params) {
  let digest;
  try {
    digest = (0, import_viem11.hashTypedData)({
...
async function verifyECDSA(address, digest, signature) {
  ...
  try {
    const recovered = await (0, import_viem11.recoverAddress)({ hash: digest, signature });
    return (0, import_viem11.isAddressEqual)((0, import_viem11.getAddress)(recovered), (0, import_viem11.getAddress)(address));
  } catch {
    return false;
  }
}
...
async function createUptoPermit2Payload(signer, x402Version, paymentRequirements) {
  ...
  const permit2Authorization = {
    from: signer.address,
    permitted: {
      token: (0, import_viem15.getAddress)(paymentRequirements.asset),
      amount: paymentRequirements.amount
    },
    ...
    deadline,
    witness: {
      to: (0, import_viem15.getAddress)(paymentRequirements.payTo),
      facilitator: (0, import_viem15.getAddress)(facilitatorAddress),
      validAfter
    }
  };
  ...
  const signature = await signer.signTypedData({
    ...
    message: {
      permitted: {
        token: (0, import_viem15.getAddress)(permit2Authorization.permitted.token),
        amount: BigInt(permit2Authorization.permitted.amount)
      },
      spender: (0, import_viem15.getAddress)(permit2Authorization.spender),
      nonce: BigInt(permit2Authorization.nonce),
      deadline: BigInt(permit2Authorization.deadline),
      witness: {
        to: (0, import_viem15.getAddress)(permit2Authorization.witness.to),
        facilitator: (0, import_viem15.getAddress)(permit2Authorization.witness.facilitator),
        validAfter: BigInt(permit2Authorization.witness.validAfter)
      }
    }
  });
...
var CHANNEL_CONFIG_TYPEHASH = (0, import_viem16.keccak256)(
  (0, import_viem16.toBytes)(
...
var ErrInvalidChannelId = "invalid_batch_settlement_evm_channel_id_invalid";
...
// src/batch-settlement/utils.ts
var CHANNEL_ID_RE = /^0x[0-9a-fA-F]{64}$/;
...
function computeChannelId(config, networkOrChainId) {
  ...
  return (0, import_viem17.hashTypedData)({
...
function getBatchSettlementEip712Domain(chainId) {
  return {
    ...
    chainId,
    verifyingContract: (0, import_viem17.getAddress)(BATCH_SETTLEMENT_ADDRESS)
  };
}
...
function buildErc3009DepositNonce(channelId, salt) {
  return (0, import_viem18.keccak256)(
    (0, import_viem18.encodeAbiParameters)([{ type: "bytes32" }, { type: "uint256" }], [channelId, BigInt(salt)])
  );
}
...
async function createBatchSettlementEIP3009DepositPayload(signer, x402Version, paymentRequirements, channelConfig, depositAmount, maxClaimableAmount, voucherSigner) {
  ...
  const signature = await signer.signTypedData({
    domain: {
      ...
      version,
      chainId,
      verifyingContract: (0, import_viem19.getAddress)(paymentRequirements.asset)
    },
    types: receiveAuthorizationTypes,
    primaryType: "ReceiveWithAuthorization",
    message: {
      from: (0, import_viem19.getAddress)(signer.address),
      to: (0, import_viem19.getAddress)(ERC3009_DEPOSIT_COLLECTOR_ADDRESS),
...
async function createBatchSettlementPermit2DepositPayload(signer, x402Version, paymentRequirements, channelConfig, depositAmount, maxClaimableAmount, voucherSigner) {
  ...
  const permit2Authorization = {
    from: signer.address,
    permitted: {
      token: (0, import_viem20.getAddress)(paymentRequirements.asset),
      amount: depositAmount
    },
    spender: (0, import_viem20.getAddress)(PERMIT2_DEPOSIT_COLLECTOR_ADDRESS),
    nonce,
    deadline,
    witness: {
      channelId
...
function buildChannelConfig(deps, paymentRequirements) {
  ...
  if (!receiverAuthorizer || (0, import_viem21.getAddress)(receiverAuthorizer) === "0x0000000000000000000000000000000000000000") {
    throw new Error("Payment requirements must include a non-zero extra.receiverAuthorizer");
  }
  return {
    payer: deps.signer.address,
    payerAuthorizer: (0, import_viem21.getAddress)(
      deps.payerAuthorizer ?? deps.voucherSigner?.address ?? deps.signer.address
    ),
    receiver: paymentRequirements.payTo,
    receiverAuthorizer: (0, import_viem21.getAddress)(receiverAuthorizer),
...
async function recoverFromSignature(deps, accept, channelState, voucherState) {
  ...
  const recovered = await (0, import_viem22.recoverTypedDataAddress)({
    domain: getBatchSettlementEip712Domain(chainId),
    types: voucherTypes,
    primaryType: "Voucher",
    message: {
  ...
  const expectedSigner = (0, import_viem22.getAddress)(
    deps.payerAuthorizer ?? deps.voucherSigner?.address ?? deps.signer.address
  );
...
var BatchSettlementEvmScheme = class {
  ...
  constructor(signer, optionsOrPolicy) {
    ...
    if (payerAuthorizer !== void 0 && voucherSigner !== void 0 && (0, import_viem23.getAddress)(payerAuthorizer) !== (0, import_viem23.getAddress)(voucherSigner.address)) {
...
function computePayerAgnosticPaymentInfoHash(chainId, paymentInfo) {
  ...
  const paymentInfoHash = (0, import_viem24.keccak256)(paymentInfoEncoded);
  const outerEncoded = (0, import_viem24.encodeAbiParameters)(
    [
  ...
  return (0, import_viem24.keccak256)(outerEncoded);
}
async function signERC3009(signer, authorization, extra, tokenAddress, chainId) {
  const domain = {
    ...
    chainId,
    verifyingContract: (0, import_viem24.getAddress)(tokenAddress)
  };
  const message = {
    from: (0, import_viem24.getAddress)(authorization.from),
    to: (0, import_viem24.getAddress)(authorization.to),
    value: BigInt(authorization.value),
    validAfter: BigInt(authorization.validAfter),
    validBefore: BigInt(authorization.validBefore),
    nonce: authorization.nonce
...
async function signPermit2(signer, permit, chainId) {
  ...
  const message = {
    permitted: {
      token: (0, import_viem24.getAddress)(permit.permitted.token),
      amount: BigInt(permit.permitted.amount)
    },
    spender: (0, import_viem24.getAddress)(permit.spender),
...
function generateSalt() {
  ...
  return (0, import_viem24.toHex)(bytes);
}
  ...
  async createPaymentPayload(x402Version, requirements, _) {
    ...
    if (assetTransferMethod === "permit2") {
      const permit2Authorization = {
        ...
        nonce: (0, import_viem25.hexToBigInt)(nonce).toString(),
