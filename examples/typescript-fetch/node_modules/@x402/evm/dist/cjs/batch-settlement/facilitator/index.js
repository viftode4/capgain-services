...
var ErrAuthorizerNotConfigured = "invalid_batch_settlement_evm_authorizer_not_configured";
var ErrDepositSimulationFailed = "invalid_batch_settlement_evm_deposit_simulation_failed";
var ErrFactoryNotAllowed = "invalid_batch_settlement_evm_eip6492_factory_not_allowed";
var ErrSmartWalletDeploymentFailed = "invalid_batch_settlement_evm_smart_wallet_deployment_failed";
...
async function verifyTypedDataSignature(signer, params) {
  let digest;
  try {
    digest = (0, import_viem4.hashTypedData)({
...
async function verifyECDSA(address, digest, signature) {
  ...
  try {
    const recovered = await (0, import_viem4.recoverAddress)({ hash: digest, signature });
    return (0, import_viem4.isAddressEqual)((0, import_viem4.getAddress)(recovered), (0, import_viem4.getAddress)(address));
  } catch {
    return false;
  }
}
...
function computeChannelId(config, networkOrChainId) {
  ...
  return (0, import_viem5.hashTypedData)({
...
function getBatchSettlementEip712Domain(chainId) {
  return {
    ...
    chainId,
    verifyingContract: (0, import_viem5.getAddress)(BATCH_SETTLEMENT_ADDRESS)
  };
}
...
function toContractChannelConfig(config) {
  return {
    payer: (0, import_viem6.getAddress)(config.payer),
    payerAuthorizer: (0, import_viem6.getAddress)(config.payerAuthorizer),
    receiver: (0, import_viem6.getAddress)(config.receiver),
    receiverAuthorizer: (0, import_viem6.getAddress)(config.receiverAuthorizer),
    token: (0, import_viem6.getAddress)(config.token),
    withdrawDelay: config.withdrawDelay,
    salt: config.salt
  };
}
...
async function verifyBatchSettlementVoucherTypedData(signer, params, chainId) {
  ...
  if (params.payerAuthorizer !== zeroAddress) {
    try {
      const digest = (0, import_viem6.hashTypedData)({
        domain,
        types: voucherTypes,
        primaryType: "Voucher",
        message
      });
      const recovered = await (0, import_viem6.recoverAddress)({
        hash: digest,
        signature: params.signature
      });
      return (0, import_viem6.isAddressEqual)(recovered, (0, import_viem6.getAddress)(params.payerAuthorizer));
    } catch {
      return false;
    }
  }
  return verifyTypedDataSignature(signer, {
    address: (0, import_viem6.getAddress)(params.payer),
    domain,
    types: voucherTypes,
    primaryType: "Voucher",
    message,
...
function validateChannelConfig(config, channelId, requirements) {
  ...
  if ((0, import_viem6.getAddress)(config.receiver) !== (0, import_viem6.getAddress)(requirements.payTo)) {
    return ErrReceiverMismatch;
  }
  const extra = requirements.extra;
  const requiredReceiverAuthorizer = extra?.receiverAuthorizer;
  if (!requiredReceiverAuthorizer || (0, import_viem6.getAddress)(requiredReceiverAuthorizer) === "0x0000000000000000000000000000000000000000" || (0, import_viem6.getAddress)(config.receiverAuthorizer) !== (0, import_viem6.getAddress)(requiredReceiverAuthorizer)) {
    return ErrReceiverAuthorizerMismatch;
  }
  if ((0, import_viem6.getAddress)(config.token) !== (0, import_viem6.getAddress)(requirements.asset)) {
...
async function readChannelState(signer, channelId) {
  const target = (0, import_viem6.getAddress)(BATCH_SETTLEMENT_ADDRESS);
...
function buildErc3009DepositNonce(channelId, salt) {
  return (0, import_viem7.keccak256)(
    (0, import_viem7.encodeAbiParameters)([{ type: "bytes32" }, { type: "uint256" }], [channelId, BigInt(salt)])
  );
}
...
function getEip3009DepositCollectorAddress() {
  return (0, import_viem8.getAddress)(ERC3009_DEPOSIT_COLLECTOR_ADDRESS);
}
function buildEip3009DepositCollectorData(payload) {
  ...
  const { signature } = (0, import_viem8.parseErc6492Signature)(auth.signature);
  return buildErc3009CollectorData(auth.validAfter, auth.validBefore, auth.salt, signature);
}
async function verifyEip3009DepositAuthorization(signer, payload, requirements, chainId, allowedFactories = []) {
  ...
  const reject = (invalidReason) => ({
    response: { isValid: false, invalidReason, payer },
    counterfactual: null
  });
  if (!auth) {
    return reject(ErrErc3009AuthorizationRequired);
  }
  const extra = requirements.extra;
  if (!extra?.name || !extra?.version) {
    return reject(ErrMissingEip712Domain);
  }
  ...
  const {
    ...
    signature: innerSig
  } = (0, import_viem8.parseErc6492Signature)(auth.signature);
  const hasDeploymentInfo = !!(factory && factoryCalldata && !(0, import_viem8.isAddressEqual)(factory, ZERO_ADDRESS));
  if (hasDeploymentInfo) {
    ...
    if (!code || code === "0x") {
      ...
      if (!isAllowed) {
        return reject(ErrFactoryNotAllowed);
      }
      return {
        response: null,
        counterfactual: { factory, factoryCalldata }
      };
    }
  }
  ...
  if (!receiveAuthOk) {
    return reject(ErrInvalidReceiveAuthorizationSignature);
  }
  return { response: null, counterfactual: null };
}
async function verifyReceiveAuth(signer, params) {
  return verifyTypedDataSignature(signer, {
    address: (0, import_viem8.getAddress)(params.payer),
    domain: {
      name: params.name,
      version: params.version,
      chainId: params.chainId,
      verifyingContract: (0, import_viem8.getAddress)(params.asset)
    },
    types: receiveAuthorizationTypes,
    primaryType: "ReceiveWithAuthorization",
    message: {
      from: (0, import_viem8.getAddress)(params.payer),
      to: (0, import_viem8.getAddress)(ERC3009_DEPOSIT_COLLECTOR_ADDRESS),
...
async function validateErc20ApprovalForPayment(info, payer, tokenAddress) {
  ...
  try {
    ...
    const tx = (0, import_viem13.parseTransaction)(serializedTx);
    if (!tx.to || (0, import_viem13.getAddress)(tx.to) !== tokenAddress) {
      return {
        isValid: false,
    ...
    try {
      const decoded = (0, import_viem13.decodeFunctionData)({
        abi: erc20ApproveAbi,
        data
      });
      const calldataSpender = (0, import_viem13.getAddress)(decoded.args[0]);
      if (calldataSpender !== (0, import_viem13.getAddress)(PERMIT2_ADDRESS)) {
        return {
          isValid: false,
    ...
    try {
      const recoveredAddress = await (0, import_viem13.recoverTransactionAddress)({
        serializedTransaction: serializedTx
      });
      if ((0, import_viem13.getAddress)(recoveredAddress) !== (0, import_viem13.getAddress)(payer)) {
        return {
          isValid: false,
...
function validateEip2612PermitForPayment(info, payer, tokenAddress) {
  ...
  if ((0, import_viem14.getAddress)(info.from) !== (0, import_viem14.getAddress)(payer)) {
    return { isValid: false, invalidReason: ErrEip2612FromMismatch };
  }
  if ((0, import_viem14.getAddress)(info.asset) !== tokenAddress) {
    return { isValid: false, invalidReason: ErrEip2612AssetMismatch };
  }
  if ((0, import_viem14.getAddress)(info.spender) !== (0, import_viem14.getAddress)(PERMIT2_ADDRESS)) {
...
function getPermit2DepositCollectorAddress() {
  return (0, import_viem15.getAddress)(PERMIT2_DEPOSIT_COLLECTOR_ADDRESS);
}
function buildPermit2DepositCollectorData(payload, eip2612PermitData = "0x") {
  ...
  const { signature } = (0, import_viem15.parseErc6492Signature)(auth.signature);
...
async function resolvePermit2DepositBranch(signer, payment, payload, requirements, context) {
  const payer = payload.channelConfig.payer;
  const tokenAddress = (0, import_viem15.getAddress)(requirements.asset);
...
function buildDepositTransaction(payload, collectorData, dataSuffix) {
  ...
  return {
    to: (0, import_viem15.getAddress)(BATCH_SETTLEMENT_ADDRESS),
...
async function verifyPermit2TypedData(signer, payload, requirements, chainId) {
  ...
  if ((0, import_viem15.getAddress)(auth.from) !== (0, import_viem15.getAddress)(payer)) {
    return { isValid: false, invalidReason: ErrPermit2InvalidSignature, payer };
  }
  if ((0, import_viem15.getAddress)(auth.spender) !== getPermit2DepositCollectorAddress()) {
    return { isValid: false, invalidReason: ErrPermit2InvalidSpender, payer };
  }
  if ((0, import_viem15.getAddress)(auth.permitted.token) !== (0, import_viem15.getAddress)(requirements.asset)) {
  ...
  const ok = await verifyTypedDataSignature(signer, {
    address: (0, import_viem15.getAddress)(auth.from),
    domain: { name: "Permit2", chainId, verifyingContract: PERMIT2_ADDRESS },
    types: batchPermit2WitnessTypes,
    primaryType: "PermitWitnessTransferFrom",
    message: {
      permitted: {
        token: (0, import_viem15.getAddress)(auth.permitted.token),
        amount: BigInt(auth.permitted.amount)
      },
      spender: (0, import_viem15.getAddress)(auth.spender),
...
async function verifyDeposit(signer, payment, payload, requirements, context, allowedFactories = []) {
  const payer = payload.channelConfig.payer;
  ...
  let erc3009Counterfactual = null;
  ...
  if (erc3009Counterfactual) {
    ...
    if (!simulationSucceeded) {
      return { isValid: false, invalidReason: ErrDepositSimulationFailed, payer };
    }
...
async function verifySharedDepositState(signer, payload, requirements) {
  ...
  const mcResults = await multicall(signer.readContract.bind(signer), [
    ...
    {
      address: (0, import_viem16.getAddress)(requirements.asset),
      abi: erc20BalanceOfABI,
      functionName: "balanceOf",
      args: [(0, import_viem16.getAddress)(payer)]
    },
    {
      address: (0, import_viem16.getAddress)(BATCH_SETTLEMENT_ADDRESS),
      abi: batchSettlementABI,
...
async function settleDeposit(signer, payment, payload, requirements, context, dataSuffix, allowedFactories = []) {
  const { deposit, voucher } = payload;
  const config = payload.channelConfig;
  const payer = config.payer;
...
async function simulateCounterfactualErc3009Deposit(signer, counterfactual, payload, depositAmount, execution) {
...
async function deployErc3009CounterfactualIfNeeded(signer, payload, requirements, allowedFactories) {
  const config = payload.channelConfig;
  const payer = config.payer;
  const auth = payload.deposit.authorization.erc3009Authorization;
  if (!auth) {
  ...
  const { address: factory, data: factoryCalldata } = (0, import_viem16.parseErc6492Signature)(auth.signature);
  const hasDeploymentInfo = !!(factory && factoryCalldata && !(0, import_viem16.isAddressEqual)(factory, ZERO_ADDRESS2));
  if (!hasDeploymentInfo) {
    return null;
  }
  let code;
...
async function executeClaimWithSignature(signer, payload, requirements, authorizerSigner, dataSuffix) {
  ...
  if (!sig) {
    ...
    for (const claim of payload.claims) {
      if ((0, import_viem17.getAddress)(claim.voucher.channel.receiverAuthorizer) !== (0, import_viem17.getAddress)(authorizerSigner.address)) {
        return {
          success: false,
  ...
  try {
    await signer.readContract({
      address: (0, import_viem17.getAddress)(BATCH_SETTLEMENT_ADDRESS),
      abi: batchSettlementABI,
...
async function executeSettle(signer, payload, requirements, dataSuffix) {
  ...
  const receiver = (0, import_viem18.getAddress)(payload.receiver);
  const token = (0, import_viem18.getAddress)(payload.token);
  try {
  ...
  try {
    ...
    let amount = "";
    if (receipt.logs) {
      const logs = (0, import_viem18.parseEventLogs)({
        abi: batchSettlementABI,
        eventName: "Settled",
        logs: receipt.logs.filter((log) => (0, import_viem18.isAddressEqual)(log.address, contractAddr))
      });
      const settledLog = logs.find(
        (log) => (0, import_viem18.isAddressEqual)(log.args.receiver, receiver) && (0, import_viem18.isAddressEqual)(log.args.token, token)
      );
...
async function executeRefundWithSignature(signer, payload, requirements, authorizerSigner, dataSuffix) {
  ...
  try {
    ...
    const contractAddr = (0, import_viem19.getAddress)(BATCH_SETTLEMENT_ADDRESS);
    ...
    if (!hasClientSig && authorizerSigner && (0, import_viem19.getAddress)(payload.channelConfig.receiverAuthorizer) !== (0, import_viem19.getAddress)(authorizerSigner.address)) {
      return {
        success: false,
        errorReason: ErrAuthorizerAddressMismatch,
        transaction: "",
    ...
    const refundSig = payload.refundAuthorizerSignature ?? await signRefund(
      authorizerSigner,
      channelId,
      payload.amount,
      payload.refundNonce,
    ...
    const refundCalldata = (0, import_viem19.encodeFunctionData)({
      abi: batchSettlementABI,
    ...
    if (payload.claims.length > 0) {
      ...
      const claimCalldata = (0, import_viem19.encodeFunctionData)({
        abi: batchSettlementABI,
        functionName: "claimWithSignature",
...
var BatchSettlementEvmScheme = class {
   ...
   */
  constructor(signer, authorizerSigner, config) {
    this.signer = signer;
