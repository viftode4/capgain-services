...
var import_utils13 = require("@x402/core/utils");
...
var ErrVerificationStateUnavailable = "invalid_batch_settlement_evm_verification_state_unavailable";
...
function channelIdBindingError(config, claimedChannelId, networkOrChainId) {
...
function verificationStateUnavailable() {
  return {
    abort: true,
    reason: ErrVerificationStateUnavailable,
...
async function handleBeforeVerify(scheme, ctx) {
  ...
  try {
    ...
    const chargedCumulativeAmount = channelSnapshot?.chargedCumulativeAmount ?? inferMissingLocalChargedAmount(
...
async function handleEnrichPaymentRequiredResponse(scheme, ctx) {
  ...
  if (channelIdBindingError(raw.channelConfig, raw.voucher.channelId, paymentPayload.accepted.network)) {
    return;
  }
...
async function handleAfterVerify(scheme, ctx) {
  ...
  try {
    updateResult = await storage.updateChannel(channelId, (current) => {
      ...
      const base = current?.chargedCumulativeAmount ?? inferMissingLocalChargedAmount(signedMaxClaimable, requirements.amount, !isRefundVoucher);
      const expectedMaxClaimable = isRefundVoucher ? BigInt(base) : BigInt(base) + BigInt(requirements.amount);
...
async function handleVerifiedPaymentCanceled(scheme, ctx) {
  if (ctx.reason !== "handler_threw" && ctx.reason !== "handler_failed" && ctx.reason !== "after_verify_aborted") {
    return;
  }
  await scheme.clearPendingRequest(ctx.paymentPayload);
}
...
async function verifyLocalVoucherSignature(raw, network) {
  try {
    return await (0, import_viem7.verifyTypedData)({
      address: (0, import_viem7.getAddress)(raw.channelConfig.payerAuthorizer),
...
var BatchSettlementEvmScheme = class {
  ...
  async enhancePaymentRequirements(paymentRequirements, supportedKind, _extensionKeys) {
    ...
    if (!receiverAuthorizer || (0, import_viem8.getAddress)(receiverAuthorizer) === "0x0000000000000000000000000000000000000000") {
    ...
    return {
      ...
      extra: {
        ...
        receiverAuthorizer: (0, import_viem8.getAddress)(receiverAuthorizer),
  ...
  validateFacilitatorSupport(network, supportedKind, _) {
    ...
    const hasValid = typeof advertised === "string" && (0, import_viem8.getAddress)(advertised) !== "0x0000000000000000000000000000000000000000";
    if (!hasValid) {
      return `no receiverAuthorizerSigner is configured and the facilitator does not advertise a receiverAuthorizer on ${network}. Configure a receiverAuthorizerSigner or use a facilitator that advertises one.`;
    }
  }
  /**
  ...
  parseMoneyToDecimal(money) {
    ...
    return (0, import_utils13.parseMoneyString)(money);
  }
  /**
  ...
  defaultMoneyConversion(amount, network) {
    ...
    const tokenAmount = (0, import_utils13.convertToTokenAmount)((0, import_utils13.numberToDecimalString)(amount), assetInfo.decimals);
